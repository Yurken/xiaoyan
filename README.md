...
<p align="center">
  面向科研学习与论文工作的桌面端 AI 研究助手<br/>
  从研究方向、文献调研到实验、写作与投稿，把断裂的科研流程重新串起来<br/>
...
- **实验 + 代码助手**：在同一工作区编写和管理实验代码，支持文件树、上下文附件、工具调用、Git 查看与审查，以及 Build、Plan 等协作模式。
- **实验快照与对比**：保存关键实验节点，按条件搜索、批量管理，并直观对比代码与结果差异。
- **研究领域动态**：从规划页查看领域简报、历史洞察与趋势变化，及时捕捉值得继续追踪的方向。
- **更顺畅的阅读和写作**：PDF 支持全文搜索、命中高亮与阅读位置恢复；LaTeX 工作区支持多文件项目与流式学术翻译。
- **角色化投稿预审**：按方法、实验、写作与相关工作等视角生成预审意见，提前发现投稿风险。
...
- **领域动态**：在研究规划中查看领域简报、历史洞察与趋势变化，持续跟进研究线索
...
- **全文搜索**：支持关键词搜索、命中高亮与快捷键跳转；切换论文后会恢复阅读位置与缩放比例
...
- **角色化 AI 预审**：基于摘要或 PDF 全文，按方法、实验、写作与相关工作等职责生成 2-4 位预审意见（含大修/小修/接收/拒稿结论）
- **投稿诊断**：投稿前风险评估与修改建议
...
- 实验可被引用为知识主张的证据
- **代码助手**：文件树、代码编辑、上下文附件、工具调用与 Git 查看/审查集中在同一工作区
- **实验快照**：保存关键节点，支持搜索、批量管理与差异对比
...
- 多文件 LaTeX 项目编辑，支持编辑器 + 预览双栏/单栏切换
- LaTeX 诊断、代码片段、字数/公式/引用统计
- AI 写作助手：润色、续写、生成摘要、审稿视角评论与流式学术翻译
...
Rust 后端 (Tauri Commands)
    ...
    ├── code/                  代码助手（上下文、Git、工具与会话状态）
    ├── agent_runtime_service.rs  # AgentRuntime（工具注册、AgentContext、统一事件边界）
...
pnpm release:landing --version v0.5.0   # 指定版本
