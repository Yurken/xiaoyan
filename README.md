...
推送 `v*` tag 触发 GitHub Actions 流水线：创建 Release → 矩阵构建（macOS / Windows / Linux）→ 上传安装包与更新包到 Cloudflare R2 → 生成更新清单（`updaters/latest.json`）与官网下载清单（`releases/latest.json`）回传 R2 → 发布。
