...
将投稿前检查提供为**多 Agent 角色化预审**：不同职责的模型独立评估论文，结果可归档为审稿意见、诊断报告和修改任务。它辅助作者自查，不冒充真实同行评审。

不同 Agent 分别扮演不同审稿视角，独立审读投稿文件；后续可由 Chair Agent 汇总冲突、生成综合意见与修改优先级。

## 当前可用实施

- 入口位于投稿版本工作区，支持从 PDF 提取正文或直接编辑内容后发起预审。
- 当前角色池包括「方法与创新」「实验与复现」「写作与相关工作」；选择四人模式时增加「领域主席」作综合录用风险判断。
- 每位角色使用独立职责提示词和结构化 JSON 输出；前端按结果流式展示，并将角色标签随意见归档。
- 预审完成后自动保存诊断报告，可把报告中的问题和建议转入投稿清单或修订任务。
...
- [x] 定义 3 个核心 Reviewer 角色（Method / Experiment / Writing）
- [x] 实现角色化串行评审编排与逐位结果流
- [x] 生成结构化文本报告（总体建议 + 优缺点 + 问题清单）
- [x] 在投稿版本工作区新增「AI 多视角预审」入口
