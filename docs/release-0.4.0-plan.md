...
- [x] 小妍对话在有研究主题上下文时，优先注入该主题的规划边界、关键论文和近期决策。
- [x] 主题页只负责组合，数据聚合进入 `features/<domain>/` hook 或后端 service。
...
- [x] 抽象论文解析 adapter，隔离 `pdf_extract`、`lopdf`、视觉扫描和未来 MinerU 接入。
- [x] 保存解析产物元数据：解析器名称、耗时、错误、正文长度、章节识别结果、图表数量、回退路径。
- [x] 在论文详情中展示解析质量摘要和重解析入口。
...
- [x] 新增 `memory_session_summaries`，记录每轮会话的目标、调查内容、已完成事项、开放问题、下一步。
- [x] 新增 `memory_links`，把记忆关联到 `paper`、`knowledge_note`、`research_interest`、`submission`、`experiment` 等实体。
- [x] 记忆检索升级为 observation + checkpoint 的混合召回，保留隐私分层校验。
- [x] 在小妍会话结束后生成轻量 checkpoint，失败或取消也记录原因。
...
- [x] AI 模拟审稿升级为“投稿前诊断报告”，覆盖录用风险、创新性、方法可靠性、实验充分性、相关工作覆盖、写作清晰度和目标刊会适配度。
- [x] 诊断报告可一键转为提交 checklist 或修改任务。
- [x] 实验记录可关联论文版本和诊断问题，形成“问题 -> 实验补强 -> 新版本”的证据链。
...
- [x] 抽象 `AgentRuntime` 边界，定义统一的 `AgentRequest`、`AgentEvent`、`AgentResult` 和取消语义；现有 `run_agentic` / `agent_graph` 先包成 `XiaoyanNativeRuntime`，不改变默认行为。
- [x] 将 `rule / llm / hybrid` 选路下沉为 `RoutingPolicy`，规则模式保留为稳定 fallback，不再作为页面或业务流程的硬编码扩展点。
- [x] 将小妍领域能力工具化，优先覆盖研究主题上下文、论文库检索、论文全文读取、Graph RAG、记忆 checkpoint、投稿诊断和实验证据查询。
- [x] 新增小妍版 context builder，集中组装小妍身份、当前研究主题、论文 / 笔记 / 实验 / 投稿上下文、记忆 checkpoint、隐私边界和工具使用约束。
- [x] 统一 Agent 事件流，把计划、节点开始、工具开始、工具完成、产物生成、文本 delta、完成和失败都收敛为后端事件模型，再映射到现有 `chat:*` 前端事件。
- [x] 协同台文案从“多个外部助手”收束为“小妍正在执行的步骤 / 工具 / 证据链”，避免用户感知到第二个助手身份。
...
- [x] 对本版本触达的大页面执行“抽离后修改”：页面只组合区块，异步数据和副作用进入 hook。
- [x] 每个新增功能域默认建立 `features/<domain>/`，共享常量、类型和纯函数进入 `shared.ts`。
- [x] 后端新增业务逻辑优先进入 service，Tauri command 只做参数校验、权限边界和调度。
- [x] 数据库 schema 变更必须包含向后兼容迁移，不破坏 0.3.x 用户数据。
- [x] 更新 `CHANGELOG.md`、版本号同步脚本流程和发布检查清单。
...
- [x] 小妍 Agent runtime 默认路径、取消逻辑、事件流和旧会话兼容性完成验证。
