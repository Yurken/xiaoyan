...
- 栏目标题必须按论文类型自然调整，不要机械套用固定模板；不适用的栏目直接省略，不要写“暂无”“不适用”凑数。
...
- 每个保留栏目至少 2-4 句实质性分析，必须结合论文实际内容，严禁空泛或复制摘要。
...
- 每个保留栏目至少 2-4 句实质性内容，以加粗标题组织，标题单独成段（标题行与内容之间用 \\n\\n 隔开）。
- 所有数学公式必须使用 LaTeX 格式：行内公式用 $...$，独立公式用 $$...$$，不得用纯文本写公式。
