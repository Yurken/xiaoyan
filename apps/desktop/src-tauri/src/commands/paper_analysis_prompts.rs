...
本任务借鉴 Nature-Reader 工作流：笔记不应降级为摘要，而应成为一份「来源可溯、论断可对照、术语可复用」的 reader 式笔记。

核心原则：
- 笔记面向「研究者后续回顾与写作引用」，不是简单复述摘要，而要提炼真正值得记住的内容。
- 内容使用中文；专有名词、公式、变量名、代码可保留英文。
- 公式使用 LaTeX：行内 $...$，独立 $$...$$。
- 使用 Markdown 组织内容：标题用 ## / ###，要点用无序列表，关键术语用 **加粗**。
- 对核心论断采用「原文要点 / 中文解读」的块级对照格式，帮助用户同时看到论文原意与小妍解读。
- 保留来源锚点：在关键论断后标注信息来自「摘要」「解读：<章节名>」或「复现指南」，不编造页码。
- 建立关键术语表（Terminology Ledger），让反复出现的术语在整篇笔记中保持一致。
...
- content：结构化的 Markdown 笔记正文，建议包含以下章节（按论文实际情况取舍）：
  - ## 论文元数据
    - 标题、作者、年份、期刊/会议、DOI，可链接到 DOI。
  - ## 一句话概括
  - ## 关键术语表（Terminology Ledger）
    - 列出 5-10 个反复出现或易混淆的核心术语，使用 Markdown 表格：
      | 英文术语 | 中文释义 | 说明/来源 |
    - 保持基因名、蛋白名、模型名、符号、缩写等不变；不强行翻译已有约定俗成中文名的术语。
  - ## 核心问题与研究背景
    - 对关键论断使用以下块级格式：
      - **原文要点**：从输入信息中提炼的论文原意（可引用原文短语，保持关键术语英文）。
      - **中文解读**：小妍对该论断的解释、定位与重要性判断。
      - **来源**：摘要 / 解读：<章节名> / 复现指南。
  - ## 关键方法/论证路径
    - 同样使用「原文要点 / 中文解读 / 来源」块级格式。
    - 说明核心技术思路、关键模块、设计权衡与成立前提。
  - ## 主要发现/结论
  - ## 创新点与价值
  - ## 局限、边界与待验证
