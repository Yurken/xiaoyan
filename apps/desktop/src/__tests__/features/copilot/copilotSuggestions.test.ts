...
import { useCopilotSuggestions } from "../../../features/copilot/welcome/useCopilotSuggestions";
import { SUGGESTION_FADE_MS, SUGGESTION_HOLD_MS } from "../../../features/copilot/welcome/shared";

afterEach(() => { vi.useRealTimers(); vi.restoreAllMocks(); });
describe("对话建议轮换", () => {
  it("先淡出旧建议，再替换文字并淡入；用户阅读或输入时保持稳定", () => {
    vi.useFakeTimers();
    const { result, rerender, unmount } = renderHook(({ busy }) => useCopilotSuggestions(busy), { initialProps: { busy: false } });
  ...
  it("减少动态效果时不自动轮播，手动切换直接完成", () => {
    ...
    const { result } = renderHook(() => useCopilotSuggestions(false));
    const first = result.current.suggestion;
    act(() => vi.advanceTimersByTime(30000));
    expect(result.current.suggestion).toBe(first);
    act(() => result.current.next());
