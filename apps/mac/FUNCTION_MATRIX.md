...
| **updatesApi** | check / install | **缺失** | — | 需新增原生 Sparkle 或手动检查 |
| **papersApi** | list/get/upload/update/delete/openFile/analyze/reproduce/listFigures/extractPdfText | 部分 | PaperRepository.swift, PaperService.swift | 缺：openFile, listFigures, extractPdfText, update(元数据) |
...
| **knowledgeApi** | listInterests/createInterest/updateInterestFolder/deleteInterestBundle/deleteInterestOnly/generateInterestHints/suggestTopics/generatePlan | 部分 | KnowledgeRepository.swift, KnowledgeService.swift | 缺：updateInterestFolder, deleteInterestBundle, deleteInterestOnly, suggestTopics |
| **knowledgeApi.notes** | listNotes/createNote/updateNote/moveNote/deleteNote/search/webClip | 部分 | KnowledgeRepository.swift, KnowledgeService.swift | 缺：moveNote, webClip, search(语义) |
...
| **chatApi** | listSessions/getSession/deleteSession/updateSessionContext/listAgentRuns/stream | 部分 | ChatRepository.swift, ChatService.swift | 缺：updateSessionContext, listAgentRuns(持久化读取) |
...
| **memoryApi** | add/list/listObservations/searchObservations/delete/clearAuto/buildContext | 部分 | MemoryRepository.swift | 缺：searchObservations, clearAuto, buildContext |
| **skillsApi** | list/create/update/delete/resetBuiltins | 部分 | SkillRepository.swift, SkillService.swift | 缺：resetBuiltins |
| **submissionApi** | 见下方详细分解 | 部分 | SubmissionRepository.swift, SubmissionService.swift | 见下方 |
| **experimentApi** | list/get/create/update/delete/attachments.{list,add,updateLabel,delete} | 部分 | ExperimentRepository.swift, ExperimentView.swift | 缺：attachments CRUD |
...
| Copilot.tsx | copilot/* | CopilotView.swift + MissionControlView + AgentStateGraphView | 完成 | 已集成状态图 |
...
| Experiment.tsx | experiment/* | ExperimentView.swift | 部分 | 缺：附件管理 |
| Tools.tsx | tools/* | ToolsView.swift + PaperDiscoveryView + PptWorkspaceView | 部分 | 已支持 .md 导出 + 结构预览；PPTX 原生生成需后端支持 |
...
### P0：结构拆分（先拆后改）
- [ ] SubmissionView.swift → SubmissionsListView/VenuesListView/VersionsView/ReviewRoundsView/AIReviewView/ChecklistView 独立文件
- [ ] CopilotView.swift → SessionSidebar/MissionControl/Composer 独立文件
- [ ] ToolsView.swift → 每个 tool 一个独立文件（多数已完成，需确认边界）
- [ ] SettingsView.swift → 每个 tab 独立文件

### P1：Schema 对齐（可重复迁移）
- [ ] 新增 v2_schema_align 迁移：补列（NOT NULL DEFAULT）、补索引、补 created_at/updated_at
...
### P2：Service 补齐
- [ ] papersApi：openFile (NSWorkspace), listFigures (PaperRepository), extractPdfText (PDFKit)
- [ ] knowledgeApi：updateInterestFolder, deleteInterestBundle/Only, suggestTopics, moveNote, webClip
- [ ] knowledgeGraph：claim/evidence/citation CRUD, snapshot, centrality/shortestPath/subgraph
- [ ] chatApi：updateSessionContext, listAgentRuns 持久化查询
- [ ] memoryApi：searchObservations, clearAuto, buildContext
- [ ] experimentApi：attachments CRUD
- [ ] exportApi：Obsidian 导出
- [ ] updatesApi：原生更新检查（至少版本对比）
...
- [x] Tools：.md 导出 + 结构预览（PPTX 原生生成需后端支持）
